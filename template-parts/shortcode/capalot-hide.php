...
<div class=" p-4 border-dashed border-2 border-rose-200 rounded-md my-4 bg-gray-100 overflow-hidden relative dark:bg-dark-card dark:border-gray-600">
  ...
  <?php else : ?>
    ...
    <div class="my-2  py-[1.5rem] px-[1rem] text-center block">
      ...
      <?php else : ?>
        <div class="text-[#ff5722] text-[1.25rem] mb-[1rem] flex justify-center items-center leading-[1.5rem]"><i class="fas fa-lock me-1"></i><?php _e('本内容需权限查看', 'ripro'); ?></div>
        <div class="mb-[1rem] text-white text-center">
          ...
          class="js-pay-action cursor-pointer bg-[#d6293e] px-[1.5rem] rounded-full hover:bg-[#b62335] p-2 block my-0 mx-[auto]" data-id="<?php echo $post_id; ?>" data-type="1" data-info="">
            ...
            <?php echo __('购买查看权限', 'ripro'); ?>
          </button>
        </div>

        <div class="text-[0.95rem] p-[0.5rem] leading-6 md:w-[550px] rounded w-full bg-[#eaf6ff] text-center block my-0 mx-[auto] dark:bg-[#293745]">
          ...
          <ul class="">
            ...
            foreach ($post_price_data as $type => $coin_price) {
              ...
              if ($coin_price === false) {
                ...
                $__price_span = '<span class=""><i class="fas' . '金币' . ' me-1"></i>' . $coin_price . '金币' . '<sup class="ms-1 ">' . sprintf(__('%s折', 'ripro'), $__rate) . '<sup></span>';
              } else {
                $__price_span = '<span class=" text-[#f7c32e] dark:text-[#f7c32e]"><span class="dashicons dashicons-database text-[1rem] "></span><i class="fas ' . '金币' . ' me-1"></i>' . $coin_price . '金币' . '</span>';
              }
        ...
        if ($sales_count > 0) {
          echo '<div class="text-[#8d9da9] text-[0.9rem] mt-[1rem] "><i class="fab fa-hotjar me-1"></i>' . sprintf(__('已有<span class="text-[#ff5722] mx-[2px] text-[0.875rem]">%d</span>人解锁查看', 'ripro'), $sales_count) . '</div>';
        }
        ?>
      <?php endif; ?>
