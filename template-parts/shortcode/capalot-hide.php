...
<div class=" p-4 border-dashed border-2 border-rose-200 rounded-md my-4 bg-gray-100 overflow-hidden relative dark:bg-dark-card dark:border-gray-600">
  <?php if ($user_pay_post_status && !$is_user_login_get_status) : ?>
    ...
    <?php echo $content; ?>
  <?php else : ?>
    <span class="absolute right-0 top-0 px-2 bg-rose-300 text-white z-40 rounded-tr-md rounded-bl-md"><?php echo '隐藏内容'; ?></span>
          ...
          <ul class="">
            ...
            foreach ($post_price_data as $type => $coin_price) {
              ...
              if ($coin_price === false) {
                ...
                $__price_span = '<span class=""><i class="fas' . '金币' . ' me-1"></i>' . $coin_price . '金币' . '<sup class="ms-1 ">' . sprintf(__('%s折', 'capalot'), $__rate) . '<sup></span>';
              } else {
